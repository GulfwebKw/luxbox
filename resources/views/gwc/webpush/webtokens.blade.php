...
<html lang="en">
	...
	<head>
		...
		<meta charset="utf-8" />
		<title>{{__('adminMessage.websiteName')}}|{{__('adminMessage.devicetoken')}}</title>
						...
						<div class="kt-subheader   kt-grid__item" id="kt_subheader">
							<div class="kt-container  kt-container--fluid ">
								<div class="kt-subheader__main">
									<h3 class="kt-subheader__title">{{__('adminMessage.devicetoken')}}</h3>
									<span class="kt-subheader__separator kt-subheader__separator--v"></span>
									<div class="kt-subheader__breadcrumbs">
										<a href="{{url('home')}}" class="kt-subheader__breadcrumbs-home"><i class="flaticon2-shelter"></i></a>
										<span class="kt-subheader__breadcrumbs-separator"></span>
										<a href="javascript:;" class="kt-subheader__breadcrumbs-link">{{__('adminMessage.devicetoken')}}</a>
							...
							<div class="kt-portlet kt-portlet--mobile">
                                ...
                                @if(auth()->guard('admin')->user()->can('webpush-list'))
									...
									<table class="table table-striped- table-bordered table-hover table-checkable " id="kt_table_1">
                                        ...
                                        @php $p=1; @endphp
                                        @foreach($webPushLists as $key=>$webPushList)
											<tr class="search-body">
												<td>{{$webPushLists->firstItem() + $key}}</td>
												<td><div style="overflow:scroll;width:800px;">{!! $webPushList->device_token !!}</div></td>
												<td>{!! $webPushList->created_at !!}</td>
												<td>{!! $webPushList->updated_at !!}</td>
													...
													<div class="modal-dialog" role="document">
														<div class="modal-content">
															...
															<div class="modal-footer">
																...
																<button type="button" class="btn btn-danger"  onClick="Javascript:window.location.href='{{url('gwc/webpush/devicetokens/delete/'.$webPushList->id)}}'">{{__('adminMessage.yes')}}</button>
															</div>
														</div>
													</div>
												</div>
                                        ...
                                        @php $p++; @endphp
                                        @endforeach   
                                        <tr><td colspan="5" class="text-center">{{ $webPushLists->links() }}</td></tr>
                                        @else
                                        <tr><td colspan="5" class="text-center">{{__('adminMessage.recordnotfound')}}</td></tr>
                                        @endif    
										</tbody>
									</table>
                                @else
