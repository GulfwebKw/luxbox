...
class DutiesController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $this->validate($request, [
            'display_order' => 'required|numeric|unique:duties,display_order',
        ...
        $path = $request->file('image')->move('uploads/duties', $name);
        $resource = new Duty();
    ...
    public function update(Request $request, $id)
    {
        //field validation
        $this->validate($request, [
            'display_order' => 'required|numeric|unique:duties,display_order,' . $id,
            'title_en' => 'required|string',
            'title_ar' => 'required|string',
            'description_en' => 'required',
            'description_ar' => 'required',
        ...
        if ($request->file('image')) {
            ...
            $imageName = 'duty-'.md5(time()).'.'.$request->image->getClientOriginalExtension();
            //set upload directory
            $path = $request->file('image')->move('uploads/duties/', $imageName);
        }
